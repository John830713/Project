...
| `mingw32-make debug` | Clean rebuild with `DEBUG_CONSOLE=1` (AllocConsole) |
...
| `mingw32-make CXXFLAGS_EXTRA=-DDEBUG_CONSOLE=1` | Incremental debug build (skip clean) |
| `py Build.py` | Regenerate `GeneratedBuild.mk`, `GeneratedModuleRegistry.*`, merged translations |
| `Build.bat [trim]` | Finds `mingw32-make` via `MakePath.txt` or MSYS2, then `clean` + `py Build.py` + `make` + optional trim |
